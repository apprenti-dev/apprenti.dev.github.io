# apprenti.dev curriculum author — repository custom instructions
...
- Every `acceptanceCriteria` item is checkable, not judged, and paired with a
  matching `evidence` string.
- Every `evidence` item is inspectable, never a claim.
- A newly referenced competency is reachable by at least three tasks in the path.
...
- `apprenti.dev/docs` — the full docs hub, including a "For Content Creators" section
  ...
  levels, writing tasks that produce real evidence) that "Writing quality content"
...
`AGENTS.md` for Codex and OpenCode, and this `.github/copilot-instructions.md` plus
a path-scoped `.github/instructions/apprenti-curriculum-author.instructions.md`)
generated from the same source in the apprentiapp repository's `skills/` directory.
If your project already has its own `copilot-instructions.md`, merge this content
into it rather than overwriting yours.*
